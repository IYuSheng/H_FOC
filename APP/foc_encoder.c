...
float hall_get_electrical_angle_rad(uint8_t hall_state)
{
    if (hall_state < 8)
    {
        return hall_elec_angle_table[hall_state];
    }
    else
    {
...
float hall_update_position_and_speed(uint32_t current_time)
{
    ...
    // 获取时间间隔
    float time_interval = (current_time - last_hall_time) * 0.001f;
    
    // 如果HALL状态发生变化且时间间隔有效
    if (current_hall_state != last_hall_state && time_interval > 0.0001f)  // 避免除以极小值
    {
        // 判断旋转方向 (1:正转/反转:-1)
    ...
    else if (current_time - last_hall_time > 100)  // 超过100ms无变化，认为静止
    {
        electrical_angle = hall_get_electrical_angle_rad(current_hall_state);
