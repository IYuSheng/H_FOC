...
// 辨识结果
static motor_param_t  g_motor_param;
static motor_ident_t  g_ident;
...
/**
 * @brief 电机参数辨识初始化并启动
 * @param v_align 对齐/锁定用D轴直流电压
 * @param v_rs Rs测试D轴直流电压
 * @param hf_freq 高频测试频率(Hz)
 * @param v_hf 高频注入幅值
 * @param v_lock_d Lq测试时的D轴锁定电压（抑制转子抖动）
 */
...
void foc_motor_parameter_ident_step(void)
{
    ...
    switch (g_ident.st)
    {
        ...
        case PARAM_ID_FLUX_RUN:
        {
            ...
            // 获取电机电角速度 (rad/s)
            float we = hall_data.elec_speed;  // 电角速度（rad/s）
